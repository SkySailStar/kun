...
import com.sevnce.auth.job.entity.vo.JobInfoByUserIdVO;
...
import com.sevnce.auth.role.service.SysRoleOuterService;
...
import com.sevnce.auth.user.mapper.SysUserOuterMapper;
...
import com.sevnce.auth.user.service.SysUserRoleOuterService;
import com.sevnce.base.api.entity.auth.dto.UserListDTO;
import com.sevnce.base.api.entity.auth.vo.*;
...
import com.sevnce.base.core.global.util.obj.ObjHelp;
...
public class SysUserOuterServiceImpl extends ServiceImpl<SysUserOuterMapper, SysUserOuter> implements SysUserOuterService {
    ...
    public SysUserOuter add(UserAddDTO dto) {
        ...
        wrapper.lambda().eq(SysUserOuter::getLoginName, dto.getLoginName()).ne(SysUserOuter::getStatus,"cancellation");
...
//        }
    ...
    public SysUserCacheInfoVO getUserCacheInfoByUserId(Long userId) {
        ...
        // 去redis获取用户权限信息
        SysUserCacheInfoVO obj = RedisHelp.getHash(AuthCacheConstants.USER_OUTER_HASH, CastHelp.toStr(userId), SysUserCacheInfoVO.class);
        if (ObjHelp.isEmpty(obj)) {
...
//        }
    ...
    public UserMenuPermissVO selectMenuListByUserId(UserMenuPermissDTO dto) {
        ...
        if (StrUtil.equals(AuthConstants.SYS_ADMIN, AuthHelp.getLoginName())) {
            ...
            userMenuPermissDTO.setUserId(CastHelp.toLong(AuthHelp.getUserId()));
            loginUserVo = sysMenuService.menuListByUserOuterId(userMenuPermissDTO);
        }
        // 查询当前用户-菜单权限列表
    ...
    public UserProjectPermissVO selectProjectListByUserId(Long userId) {
        ...
        if(StrUtil.equals(AuthConstants.SYS_ADMIN, AuthHelp.getLoginName())) {
            ...
            loginVoList = sysProjectService.selectProjectNoByUserOuterId(CastHelp.toLong(AuthHelp.getUserId()));
        }
    ...
    public UserRolePermissVO selectRoleListByUserId(Long userId) {
        ...
        if (StrUtil.equals(AuthConstants.SYS_ADMIN, AuthHelp.getLoginName())) {
            ...
            loginUserVo = sysRoleOuterService.selectListByUserId(CastHelp.toLong(AuthHelp.getUserId()));
     ...
     */
    @Override
    public Boolean edit(UserBlackListDTO dto) {

        SysUserOuter sysUserOuter = new SysUserOuter();
        // 传入值复制到数据库对象
        BeanUtil.copyProperties(dto, sysUserOuter);
        sysUserOuter.setId(dto.getId());
        sysUserOuter.setStatus(dto.getStatus());
        // 修改对象
        boolean edit = updateById(sysUserOuter);
        // 判断是否修改成功
        if (edit) {
            log.info("黑名单-修改成功");
        }
            ...
            log.warn("黑名单-修改失败");
            throw new BusinessException(ErrorCodeConstants.EDIT_FAIL,"黑名单-修改失败");
        }
        return edit;
    }
     ...
     */
    @Override
    public Boolean logOff(UserBlackListDTO dto) {
        ...
        if (edit) {
            log.info("用户-注销成功");
            // 删除用户缓存
            RedisHelp.delHash(AuthCacheConstants.USER_OUTER_HASH, Convert.toStr(dto.getId()));

            // 删除用户角色
            QueryWrapper<SysUserRoleOuter> sysUserRoleOuterQueryWrapper = new QueryWrapper<>();
            sysUserRoleOuterQueryWrapper.lambda().eq(SysUserRoleOuter::getUserOuterId,dto.getId());
            sysUserRoleOuterService.remove(sysUserRoleOuterQueryWrapper);

            // 删除用户职位
            QueryWrapper<SysUserJobOuter> sysUserJobOuterQueryWrapper = new QueryWrapper<>();
            sysUserJobOuterQueryWrapper.lambda().eq(SysUserJobOuter::getUserOuterId,dto.getId());
            sysUserJobOuterService.remove(sysUserJobOuterQueryWrapper);

            // 删除用户菜单
            QueryWrapper<SysUserMenuOuter> sysUserMenuOuterQueryWrapper = new QueryWrapper<>();
            sysUserMenuOuterQueryWrapper.lambda().eq(SysUserMenuOuter::getUserOuterId,dto.getId());
            sysUserMenuOuterService.remove(sysUserMenuOuterQueryWrapper);

            // 删除用户项目
            QueryWrapper<SysUserProjectOuter> sysUserProjectOuterQueryWrapper = new QueryWrapper<>();
            sysUserProjectOuterQueryWrapper.lambda().eq(SysUserProjectOuter::getUserOuterId,dto.getId());
            sysUserProjectOuterService.remove(sysUserProjectOuterQueryWrapper);
        }
        else {
            log.warn("用户-注销失败");
            throw new BusinessException(ErrorCodeConstants.EDIT_FAIL,"用户-注销失败");
        }
        return edit;
    }
}
