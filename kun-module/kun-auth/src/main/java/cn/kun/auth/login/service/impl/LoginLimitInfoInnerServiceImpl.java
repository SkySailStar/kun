...
import cn.kun.auth.login.mapper.LoginLimitInfoInnerMapper;
import cn.kun.auth.login.service.LoginLimitInfoInnerService;
...
public class LoginLimitInfoInnerServiceImpl extends ServiceImpl<LoginLimitInfoInnerMapper, LoginLimitInfoInner> implements LoginLimitInfoInnerService {
    ...
    public Page<LoginLimitPageVO> page(LoginLimitPageDTO dto) {
        ...
        if (!StrUtil.equals(BaseConstants.ADMIN_LOGIN_NAME,loginName)){
    ...
    public LoginLimitInfoInner add(LoginLimitAddDTO dto) {

        log.info("用户登录限制信息-开始添加：{}", dto);
        // 校验登录类型
        QueryWrapper<LoginLimitInfoInner> qw = new QueryWrapper<>();
        qw.lambda().eq(LoginLimitInfoInner::getLoginType, dto.getLoginType());
        qw.lambda().eq(LoginLimitInfoInner::getLoginName,dto.getLoginName());
        long count = count(qw);
        if (count >= 1) {
            log.info("登录类型重复");
        ...
        /* 传入值复制到数据库对象 */
        LoginLimitInfoInner loginLimitInfoInner = new LoginLimitInfoInner();
        BeanUtil.copyProperties(dto, loginLimitInfoInner);
        loginLimitInfoInner.setUserInnerIp(dto.getIp());
        boolean add = save(loginLimitInfoInner);
        if (add) {
        ...
        return loginLimitInfoInner;
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public LoginLimitInfoInner edit(LoginLimitEditDTO dto) {

        log.info("用户登录限制信息-开始修改：{}", dto);
        /* 获取数据库对象 */
        LoginLimitInfoInner loginLimit = getById(dto.getId());
    ...
    public CheckResultVO isLimited(CheckDTO dto) {
        ...
        // 查询是否有开启的状态
        QueryWrapper<LoginLimitInfoInner> qw = new QueryWrapper<>();
        qw.lambda().eq(LoginLimitInfoInner::getStatus,"1");
        qw.lambda().isNull(LoginLimitInfoInner::getLoginName);
        LoginLimitInfoInner data = getOne(qw);
        // 获取当前时间
        ...
        if (ObjUtil.isNotEmpty(data)) {
            ...
            if (startTime.isBefore(nowTime) && nowTime.isBefore(endTime)){
                ...
                else if (StrUtil.isNotBlank(data.getUserInnerIp())){
                    // 校验ip
                    String ip = dto.getIp();
...
//                else if (StrUtil.isNotBlank(data.getUserInnerIp()) && StrUtil.isBlank(data.getLoginType())){
//                    // 校验ip
//                    String ip = dto.getIp();
//                    String regex = "^((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.){3}(\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5]|[*])$";
//                    Pattern p = Pattern.compile(regex);
...
//                }
        ...
        qw.lambda().eq(LoginLimitInfoInner::getLoginName,dto.getLoginName());
        qw.lambda().eq(LoginLimitInfoInner::getStatus,"1");
        LoginLimitInfoInner obj = getOne(qw);
        /* 判断当前登录用户是否单独被限制 */
        if (ObjUtil.isNotEmpty(obj)){
            ...
            if (startTime.isBefore(nowTime) && nowTime.isBefore(endTime)){
                ...
                else if (StrUtil.isNotBlank(obj.getUserInnerIp()) && StrUtil.equals(obj.getLoginType(), dto.getLoginType()) && (obj.getStartDate().isBefore(nowTime) && nowTime.isBefore(obj.getEndDate()))) {
                    // 校验ip
                    String ip = dto.getIp();
                    String regex = "^((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.){3}(\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5]|[*])$";
                    Pattern p = Pattern.compile(regex);
...
//                else if (StrUtil.isNotBlank(obj.getUserInnerIp()) && StrUtil.isBlank(obj.getLoginType()) && (obj.getStartDate().isBefore(nowTime) && nowTime.isBefore(obj.getEndDate()))) {
//                    // 校验ip
//                    String ip = dto.getIp();
//                    String regex = "^((\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5])\\.){3}(\\d|[1-9]\\d|1\\d\\d|2[0-4]\\d|25[0-5]|[*])$";
//                    Pattern p = Pattern.compile(regex);
...
//                }
    ...
    public void setStatus(SetStatusDTO dto) {
        ...
        // 创建更新对象
        UpdateWrapper<LoginLimitInfoInner> uw = new UpdateWrapper<>();
        uw.lambda().eq(LoginLimitInfoInner::getId, dto.getId());
        /* 对象赋值 */
        LoginLimitInfoInner loginLimitInfoInner = new LoginLimitInfoInner();
        loginLimitInfoInner.setStatus(dto.getStatus());
        // 修改状态
        boolean update = update(loginLimitInfoInner, uw);
        if (update) {
