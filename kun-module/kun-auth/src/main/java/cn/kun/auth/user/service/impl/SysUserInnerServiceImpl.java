...
import com.sevnce.auth.user.entity.po.SysUserDetailInner;
...
import com.sevnce.auth.user.mapper.SysUserInnerMapper;
import com.sevnce.auth.user.service.SysUserDetailInnerService;
...
//        }
    ...
    public SysUserInner add(UserAddDTO dto) {
        ...
        wrapper.lambda().eq(SysUserInner::getLoginName, dto.getLoginName()).ne(SysUserInner::getStatus,"cancellation");
...
//        }
    ...
    public SysUserCacheInfoVO getUserCacheInfoByUserId(Long userId) {
        // 去redis获取用户权限信息
        SysUserCacheInfoVO obj = RedisHelp.getHash(AuthCacheConstants.USER_INNER_HASH, userId.toString(), SysUserCacheInfoVO.class);
...
//        }
    ...
    public UserRolePermissVO selectRoleListByUserId(Long userId) {
        ...
        if (StrUtil.equals(AuthConstants.SYS_ADMIN, loginName)) {
            ...
            loginUserVo = sysRoleInnerService.selectListByUserId(CastHelp.toLong(AuthHelp.getUserId()));
    ...
    public UserProjectPermissVO selectProjectListByUserId(Long userId) {
        ...
        if(StrUtil.equals(AuthConstants.SYS_ADMIN, AuthHelp.getLoginName())) {
            ...
            loginVoList = sysProjectService.selectProjectNoByUserInnerId(CastHelp.toLong(AuthHelp.getUserId()));
        }
    ...
    public Boolean updateUserPassword(UserPasswordDTO dto) {
        ...
        // 更新缓存信息
        log.info("内部用户-修改成功-更新缓存");
        selectUserInnerPowerInfo(CastHelp.toLong(AuthHelp.getUserId()));
        return updateById(sysUserInner);
    }
    ...
    public Boolean edit(UserBlackListDTO dto) {
        ...
        // 传入值复制到数据库对象
        BeanUtil.copyProperties(dto, sysUserInner);
        sysUserInner.setId(dto.getId());
        sysUserInner.setStatus(dto.getStatus());
        // 修改对象
        boolean edit = updateById(sysUserInner);
        // 判断是否修改成功
        if (edit) {
            log.info("黑名单-修改成功");
        }
    ...
    public Boolean logOff(UserBlackListDTO dto) {
        ...
        if (edit) {
            ...
            log.info("用户-注销成功");
            // 删除用户缓存
            RedisHelp.delHash(AuthCacheConstants.USER_INNER_HASH, Convert.toStr(dto.getId()));

            // 删除用户角色
            QueryWrapper<SysUserRoleInner> sysUserRoleInnerQueryWrapper = new QueryWrapper<>();
            sysUserRoleInnerQueryWrapper.lambda().eq(SysUserRoleInner::getUserInnerId,dto.getId());
            sysUserRoleInnerService.remove(sysUserRoleInnerQueryWrapper);

            // 删除用户职位
            QueryWrapper<SysUserJobInner> sysUserJobInnerQueryWrapper = new QueryWrapper<>();
            sysUserJobInnerQueryWrapper.lambda().eq(SysUserJobInner::getUserInnerId,dto.getId());
            sysUserJobInnerService.remove(sysUserJobInnerQueryWrapper);

            // 删除用户菜单
            QueryWrapper<SysUserMenuInner> sysUserMenuQueryWrapper = new QueryWrapper<>();
            sysUserMenuQueryWrapper.lambda().eq(SysUserMenuInner::getUserInnerId,dto.getId());
            sysUserMenuInnerService.remove(sysUserMenuQueryWrapper);

            // 删除用户项目
            QueryWrapper<SysUserProjectInner> sysUserProjectInnerQueryWrapper = new QueryWrapper<>();
            sysUserProjectInnerQueryWrapper.lambda().eq(SysUserProjectInner::getUserInnerId,dto.getId());
            sysUserProjectInnerService.remove(sysUserProjectInnerQueryWrapper);
        }
        else {
            log.warn("用户-注销失败");
