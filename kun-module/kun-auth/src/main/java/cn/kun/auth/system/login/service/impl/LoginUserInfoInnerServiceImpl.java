...
import cn.kun.auth.system.login.mapper.LoginUserInfoInnerMapper;
...
public class LoginUserInfoInnerServiceImpl extends ServiceImpl<LoginUserInfoInnerMapper, LoginUserInfoInner> implements LoginUserInfoInnerService {
    ...
    public LoginUserInfoInner add(LoginUserAddDTO dto) {

        log.info("用户登录信息-开始添加：{}", dto);
        QueryWrapper<LoginUserInfoInner> qw = new QueryWrapper<>();
        qw.lambda().eq(LoginUserInfoInner::getLoginName,dto.getLoginName());
        qw.lambda().eq(LoginUserInfoInner::getLoginType,dto.getLoginType());
        LoginUserInfoInner info = getOne(qw);
        ...
        /* 新增数据：传入值复制到数据库对象 */
        LoginUserInfoInner loginUserInfoInner = new LoginUserInfoInner();
        BeanUtil.copyProperties(dto,loginUserInfoInner);

        // 添加对象
        boolean add = save(loginUserInfoInner);
        if (add) {
